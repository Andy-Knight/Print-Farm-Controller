...
**v0.16.1 is the current merged release on `main`; `feature/bambu-a1-mini` is versioned as v0.17.0 and has passed the full `npm test` regression suite with 0 failures.** Initial integrated-simulator validation has also passed for A1 Mini online/idle state, 80 °C bed limit, four AMS Lite slots plus external spool, and camera preview. The UI now hides unsupported chamber-preheat and chamber-fan controls instead of rendering them disabled on models such as A1 Mini. Print-start/job-control/material-mapping checks remain outstanding; physical A1 Mini validation remains outstanding.
