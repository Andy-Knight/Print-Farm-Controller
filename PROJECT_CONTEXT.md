...
**v0.17.0 is the current merged release on `main`; `feature/concurrent-client-safety-v0180` is versioned as v0.18.0 and adds concurrent-client safety for shared controller use.** The existing HTTP/SSE architecture already supports multiple simultaneous browser clients for monitoring. v0.18.0 now protects both overlapping controller transactions and longer-lived physical printer activity. A central compatibility matrix classifies live state as idle, printing, paused, chamber preheat, manual heating, bed levelling, calibration, fault or generic busy, then decides which mutations are allowed. Safe controls remain available where appropriate (for example camera viewing, pause/cancel during printing, resume while paused, print-time temperature/fan changes, file upload and heaters-off), while incompatible work such as starting another print during levelling/calibration/preheat is blocked with HTTP 409.

Shared mutable controller data is also hardened for concurrent clients. Print Library and queue API mutations are serialized together so library deletion cannot race a new queue reference; printer-registry and licence installation mutations are serialized; printer and file-material JSON read-modify-write operations are serialized; queue/store metadata writes use collision-safe temporary files; and Print Library initialization/mutations are guarded. Automatic queue compatibility exposes controller transactions and physical printer activity as temporary blockers. Controller-started bed levelling is tracked beyond the request until live activity returns idle; U1 tool-offset calibration is tracked across its multi-step workflow until exit (with a bounded safety timeout). U1 Moonraker `idle_timeout` activity is normalized as `machineActivity` so long-running macros are not mistaken for idle. The integrated simulator now exposes a short live bed-levelling state for Snapmaker U1 and FlashForge validation.
...
1. Re-run the full `npm test` suite after the live physical-activity matrix changes. The earlier v0.18.0 regression run was clean before this second concurrency-hardening step, so it must be repeated. New coverage now includes live print/pause/preheat/levelling/calibration compatibility decisions, U1 macro activity normalization, physical bed-levelling tracking, simulated levelling states, same-printer transaction conflicts, different-printer concurrency, batch/file-distribution arbitration, queue start/cancel races, active-operation queue blockers, concurrent printer-registry mutations, concurrent file-material writes and concurrent Print Library deduplication. Then validate with two simultaneous browser clients: start bed levelling or another longer printer activity from one client and confirm a conflicting print start from the other is blocked for the physical activity duration while safe monitoring/controls continue to work.
