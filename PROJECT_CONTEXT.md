...
- Current application version on this branch: **0.25.0**.
...
- **Current v0.25.0 feature branch:** `feature/printer-groups-v0250`. It adds controller-owned custom printer groups stored in `data/printer-groups.json`. Each printer may belong to zero or one group. Automatic Next Available queue jobs can optionally be restricted to a selected group, with live membership evaluated before dispatch while all existing compatibility/readiness/safety rules remain in force. Maintenance assignment now supports individual printer, custom printer group, and printer model scopes. Group-wide rules retain independent per-printer baselines, completion state and history. Printer-group state is included in logical `.pfcbackup` backup/restore.
