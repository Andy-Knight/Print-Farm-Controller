...
- **Current v0.22.0 feature branch:** `feature/colour-family-matching`. Queue colour compatibility treats slicer hex values as shade metadata within practical colour families rather than requiring byte-for-byte hex equality. Same-family shades can run automatically; different families remain blocked. FlashForge manual filament designation stores an authoritative colour family with an optional exact shade; existing hex-only designations automatically derive their family. Editable/non-RFID Snapmaker U1 filament setup uses the same family list and writes the family's representative hex to the printer; official RFID filament remains read-only. U1/AMS candidate mapping prefers the closest compatible shade using CIELAB colour distance without turning shade distance itself into a hard requirement.
...
- Colour compatibility is semantic rather than exact-hex: normalized slicer/printer colours must resolve to the same colour family for unattended scheduling, while the original hex values are retained and CIELAB distance is used only to prefer the closest shade among otherwise-valid candidates. FlashForge controller-side manual designation uses a required family plus optional exact shade. Editable/non-RFID U1 filament configuration also uses the canonical family list; selecting a family writes its representative hex colour into the U1, while RFID-managed filament remains controlled by the printer. Material and nozzle requirements remain strict.
