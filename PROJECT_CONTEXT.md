...
- Current application version on this branch: **0.30.1**.
...
The first implementation is manufacturer-agnostic and controller-owned. A persistent `data/maintenance.json` store tracks recurring maintenance tasks, completion history, controller-observed print seconds and observed print cycles per configured printer. Store writes remain serialized and atomic via same-directory temp-file replacement, now with bounded retry for transient Windows `EPERM`, `EACCES`, and `EBUSY` rename failures; the save chain also recovers after an individual failed write so later saves can proceed. Tasks can be scoped to one printer or defined once as a printer-model rule. Model rules are inherited automatically by all current and future matching printers without copying the task into each printer record. Each matching printer receives its own assignment timestamp, usage baseline, completion state and history, so completing a shared rule on one printer never resets another. Model rules also support a bulk **Complete for model** action: it completes the rule on every matching printer that is currently eligible, writes a normal per-printer history entry and resets that printer's own baseline; matching printers still locked before Due soon are reported and skipped. A printer added later starts that rule from its own onboarding/assignment point rather than inheriting the template's original creation date. Tasks support calendar-day, print-hour, or print-count intervals and expose Current, Due soon and Due state. Completion records optional notes and a usage snapshot while retaining history even if the task is later deleted. A newly created enabled task can be completed immediately to establish a recorded maintenance baseline, but once completed on a printer it cannot be completed again until that printer reaches Due soon (80% of its interval) or Due. This repeat-completion guard is enforced by both the UI and service/API, including inherited model-wide tasks independently per printer.

The Maintenance UI is available from the controller overflow menu and is split into three selectable views at the top: **Add maintenance tasks**, **Model-wide maintenance rules**, and **Individual printers**. It supports adding, editing, enabling/disabling, completing and deleting tasks across the fleet. Opening Maintenance normally starts on Individual printers; dashboard spanner shortcuts open the same view focused on the selected printer, while Edit actions switch automatically to the Add maintenance tasks view. It shows observed usage and recent history per printer. Each individual-printer card includes a confirmed Clear history action that removes only that printer's history entries while preserving maintenance schedules, last-completed timestamps/usage, controller-observed usage counters, recurrence baselines and due status. Usage is explicitly labelled controller-observed because it is derived from live printer activity seen by this controller rather than a manufacturer lifetime counter. Per-printer maintenance status is also included in the live fleet/SSE payload. When any printer is Due soon or Due, the top bar shows a bell alarm with the number of affected printers; selecting it filters the dashboard to only maintenance-alert printers. Dashboard printer cards now reserve a consistent header height on multi-column layouts, use a column layout with a bottom-anchored footer, and always reserve a one-line error slot beneath Open printer. This keeps camera previews and Open printer buttons aligned even when printer/model text wraps or only some printers display an offline error. Each printer card and printer-detail header shows a wrench status icon, and the printer detail Maintenance panel shows the maintenance-tracking summary. Both the dashboard-card wrench and the printer-details header wrench are interactive shortcuts that open Maintenance focused on that printer, preselect it in the add-task form, scroll to its maintenance card and briefly highlight the target. The printer-details shortcut closes the printer-details dialog before opening Maintenance. If the last alert is cleared while the maintenance filter is active, the dashboard returns automatically to the full fleet.
