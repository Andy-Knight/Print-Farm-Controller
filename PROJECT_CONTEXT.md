...
**v0.16.1 is the current merged release on `main`; `feature/bambu-a1-mini` is versioned as v0.17.0 and has passed the full `npm test` regression suite with 0 failures.** Initial integrated-simulator validation has also passed for A1 Mini online/idle state, 80 °C bed limit, four AMS Lite slots plus external spool, camera preview, single-material `.gcode` upload/start, Pause/Resume/Cancel, and plate-sliced `.3mf` upload/start through the normal controller workflow. The UI now hides unsupported chamber-preheat and chamber-fan controls instead of rendering them disabled on models such as A1 Mini. AMS Lite material mapping and final return-to-idle checks remain outstanding; physical A1 Mini validation remains outstanding.
...
1. Complete v0.17.0 A1 Mini integrated-simulator validation: verify AMS Lite source mapping and return-to-idle behavior. Already confirmed: online/idle state, 80 °C bed limit, four AMS Lite slots plus external spool, camera preview, successful single-material `.gcode` upload/start, Pause/Resume/Cancel, plate-sliced `.3mf` upload/start, and the earlier full `npm test` run with 0 failures. Unsupported chamber controls were observed as disabled during simulator validation and have been corrected to be omitted entirely; run one final full `npm test` after the remaining simulator checks because the UI changed afterward. Physical A1 Mini validation can follow when hardware is available.
