...
- **v0.17.0 experimental A1 Mini support:** `feature/bambu-a1-mini` extends the Bambu LAN adapter and integrated emulator to A1 Mini, including MQTT status/control, FTPS file operations, TLS/JPEG camera support, AMS Lite material mapping, 80 °C bed limit, 300 °C nozzle limit, no chamber controls, and a conservative `.3mf`-only controller print-start workflow until raw G-code launch is physically validated. Physical A1 Mini validation remains outstanding.
