...
- Colour compatibility is semantic rather than exact-hex: normalized slicer/printer colours must resolve to the same colour family for unattended scheduling, while the original hex values are retained and CIELAB distance is used only to prefer the closest shade among otherwise-valid candidates. FlashForge controller-side manual designation therefore uses a required family plus optional exact shade; U1 exact-colour controls remain unchanged because they write the actual printer filament configuration. Material and nozzle requirements remain strict.
