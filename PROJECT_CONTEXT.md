...
- **Current v0.27.0 feature branch:** `feature/dashboard-static-printer-images-v0270`. Dashboard printer cards now use bundled static transparent-background model artwork rather than live camera snapshots. Supported artwork covers AD5M Pro, Creator 5, Creator 5 Pro, Snapmaker U1, Bambu P1P, P1S, X1 Carbon and A1 Mini, with a neutral fallback for unknown future models. The dashboard image well is theme-aware for Light and Dark modes. Printer-detail camera streaming remains unchanged and continues to use the live `/camera/stream` endpoint plus the existing Restart camera control. Removing dashboard snapshot polling also reduces camera/network load without removing camera functionality from printer details.\n- Runtime: **Node.js 24+** (development baseline Node.js 24.21.0), ES modules, no npm runtime dependencies.
...
Local Fleet Controller (`src/`)
        ...
        +-- persistent custom printer groups + exclusive printer membership
