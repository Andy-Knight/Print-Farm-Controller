...
**v0.16.1 is the current merged release on `main`; `feature/bambu-a1-mini` is versioned as v0.17.0 and has passed the full `npm test` regression suite with 0 failures.** Initial integrated-simulator validation has also passed for A1 Mini online/idle state, 80 °C bed limit, four AMS Lite slots plus external spool, camera preview, single-material `.gcode` upload/start, and Pause/Resume/Cancel through the normal controller workflow. The UI now hides unsupported chamber-preheat and chamber-fan controls instead of rendering them disabled on models such as A1 Mini. `.3mf` start, AMS Lite material mapping and final return-to-idle checks remain outstanding; physical A1 Mini validation remains outstanding.
