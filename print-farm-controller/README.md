# Printer Fleet Controller v0.12.16

> v0.12.16 adds **native Snapmaker U1 third-party filament type selection** to each toolhead card. Idle, loaded, editable third-party slots can be assigned one of the U1 firmware's generic material profiles using stock `SET_PRINT_FILAMENT_CONFIG`; the controller verifies printer read-back and immediately uses the selected material for queue compatibility. Official RFID filament remains locked.
