...
const groupField = document.querySelector('#maintenanceGroupField');
const groupSelect = document.querySelector('#maintenanceGroup');
...
let adapters = [];
let groups = [];
let maintenance = { modelTasks:[], groupTasks:[], printers:[] };
...
function groupLabel(groupId, fallback = '') {
  return groups.find((group) => group.id === String(groupId || ''))?.name || fallback || 'Unknown group';
}

function assignmentLabel(task, printer = null) {
  if (task.assignment?.scope === 'model') return `Model · ${modelLabel(task.assignment)}`;
  if (task.assignment?.scope === 'group') return `Group · ${groupLabel(task.assignment?.groupId, task.assignment?.groupName)}`;
...
function groupTaskSection() {
  const tasks = [...(maintenance.groupTasks || [])].sort((a, b) =>
    groupLabel(a.assignment?.groupId || a.target?.groupId, a.assignment?.groupName).localeCompare(
      groupLabel(b.assignment?.groupId || b.target?.groupId, b.assignment?.groupName)
    ) || String(a.name).localeCompare(String(b.name))
  );
  if (!tasks.length) return '';
    ...
    <section class="maintenance-model-rules maintenance-group-rules">
      ...
      <div class="maintenance-model-rule-list">
        ${tasks.map((task) => {
          ...
          const eligible = Number(completion.eligible || 0);
          const locked = Number(completion.locked || 0);
          const groupName = groupLabel(task.assignment?.groupId || task.target?.groupId, task.assignment?.groupName);
          return `
          <article class="maintenance-model-rule">
            <div class="maintenance-task-main">
              <div class="maintenance-task-title-row">
                <strong>${escapeHtml(task.name)}</strong>
                <span class="maintenance-scope-pill">Group · ${escapeHtml(groupName)}</span>
                ${task.enabled === false ? '<span class="maintenance-pill" data-state="disabled">Disabled</span>' : ''}
              </div>
              ${task.description ? `<div class="maintenance-task-description">${escapeHtml(task.description)}</div>` : ''}
              <div class="maintenance-task-meta">
                <span>${escapeHtml(scheduleLabel(task))}</span>
                <span>Applies to current group members automatically</span>
                <span>${matching} matching · ${eligible} ready${locked ? ` · ${locked} locked` : ''}</span>
              </div>
            </div>
            <div class="mini-actions maintenance-task-actions">
            ...
            <div class="maintenance-history-list">
              ${history.length ? history.map((entry) => `
                <div class="maintenance-history-entry">
                  ...
                  <small class="maintenance-history-scope">${escapeHtml(entry.assignment?.scope === 'model' ? `Model · ${modelLabel(entry.assignment)}` : entry.assignment?.scope === 'group' ? `Group · ${groupLabel(entry.assignment?.groupId, entry.assignment?.groupName)}` : 'Individual printer')}</small>
...
function setMaintenanceView(view) {
  const next = ['add', 'model', 'group', 'printers'].includes(view) ? view : 'printers';
...
function populateGroups() {
  ...
  groupSelect.innerHTML = groups.length
    ? [...groups].sort((a, b) => String(a.name).localeCompare(String(b.name)))
        .map((group) => `<option value="${escapeHtml(group.id)}">${escapeHtml(group.name)} · ${Number(group.printerIds?.length || 0)} printer${Number(group.printerIds?.length || 0) === 1 ? '' : 's'}</option>`)
        .join('')
    : '<option value="">No printer groups configured</option>';
  if (groups.some((group) => group.id === previous)) groupSelect.value = previous;
}
...
function updateAssignmentFields() {
  const requested = assignmentScopeInput?.value;
  const scope = ['printer','group','model'].includes(requested) ? requested : 'printer';
  ...
  if (groupSelect) groupSelect.required = scope === 'group';
...
async function refresh() {
  ...
  try {
    const [printerResult, maintenanceResult, adapterResult, groupResult] = await Promise.all([
      api('/api/printers'),
      api('/api/maintenance'),
      api('/api/adapters'),
      api('/api/printer-groups')
    ]);
    printers = printerResult.printers || [];
    maintenance = maintenanceResult.maintenance || { modelTasks:[], groupTasks:[], printers:[] };
...
function findGroupTask(taskId) {
  return maintenance.groupTasks?.find((item) => item.id === taskId) || null;
}
...
window.addEventListener('pfc:printer-groups-changed', () => {
...
form?.addEventListener('submit', async (event) => {
  ...
  const requestedScope = assignmentScopeInput?.value;
  const scope = ['printer','group','model'].includes(requestedScope) ? requestedScope : 'printer';
  ...
  if (scope === 'model') {
    ...
    if (!groups.some((group) => group.id === groupId)) {
      if (errorEl) { errorEl.textContent = 'Choose a printer group'; errorEl.classList.remove('hidden'); }
      return;
    }
    payload.target = { groupId };
    url = taskId
      ? `/api/maintenance/group-tasks/${encodeURIComponent(taskId)}`
      : '/api/maintenance/group-tasks';
  } else {
  ...
  try {
    ...
    await refresh();
    resetForm();
    setMaintenanceView(completedScope === 'model' ? 'model' : completedScope === 'group' ? 'group' : 'printers');
  } catch (error) {
    if (errorEl) { errorEl.textContent = error.message; errorEl.classList.remove('hidden'); }
  } finally {
...
list?.addEventListener('click', async (event) => {
  ...
  const completeGroup = event.target.closest('[data-maintenance-complete-group]');
  ...
  const action = completeModel || completeGroup || complete || edit || remove;
  if (!action) return;

  const scope = completeModel || action.dataset.taskScope === 'model'
    ? 'model'
    : completeGroup || action.dataset.taskScope === 'group' ? 'group' : 'printer';
  ...
  const task = scope === 'model'
    ? (findModelTask(taskId) || effectiveTask)
    : scope === 'group' ? (findGroupTask(taskId) || effectiveTask) : effectiveTask;
  if (!task) return;
  ...
  if (completeGroup) {
    ...
    const groupName = groupLabel(task.assignment?.groupId || task.target?.groupId, task.assignment?.groupName);
    const notes = prompt(
      `Complete “${task.name}” for ${eligible} eligible printer${eligible === 1 ? '' : 's'} in ${groupName}?`
      + (locked ? `\n\n${locked} group member${locked === 1 ? '' : 's'} ${locked === 1 ? 'is' : 'are'} not yet Due soon and will be skipped.` : '')
      + '\n\nOptional maintenance notes:',
      ''
    );
    if (notes === null) return;
    completeGroup.disabled = true;
    try {
      const result = await api(`/api/maintenance/group-tasks/${encodeURIComponent(taskId)}/complete`, {
        method:'POST',
      ...
      if (statusEl) {
        statusEl.textContent = `Completed “${task.name}” on ${completedCount} printer${completedCount === 1 ? '' : 's'} in ${groupName}`
          + (skippedCount ? `; ${skippedCount} skipped because ${skippedCount === 1 ? 'it was' : 'they were'} not yet eligible.` : '.');
      }
    } catch (error) {
      completeGroup.disabled = false;
      if (errorEl) { errorEl.textContent = error.message; errorEl.classList.remove('hidden'); }
    }
    return;
  }
