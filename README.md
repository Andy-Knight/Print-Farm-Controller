...
> **Current development: v0.23.0 backup and recovery.** Portable `.pfcbackup` logical snapshots, manifest/checksum verification, manual backup creation/download, and read-only restore inspection are implemented on `feature/backup-recovery-v0230`. Restore inspection validates archive membership, checksums, controller/schema versions, state shapes, Print Library payload/hash relationships, queue references and staging-space availability without changing live data. The next stage is restore staging/startup activation with rollback and recovery-held unfinished queue work, followed by scheduled local/NAS backups. Cloud destinations and built-in backup encryption remain deferred beyond v0.23.0. See `docs/BACKUP_RECOVERY.md`.
