# Print Farm Controller v0.20.1

> **Current development: v0.20.1 printer-detail opening reliability.** The printer dialog now opens immediately with a loading state instead of waiting for the printer file-list request to finish. Slow printer storage access therefore no longer makes **Open printer** appear unresponsive. Stale asynchronous opens are discarded when the dialog is closed/reopened, and top-level open failures are surfaced instead of failing silently. Dashboard connection errors now render below **Open printer**, keeping action buttons aligned across printer cards.
