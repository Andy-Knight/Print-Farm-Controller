# Print Farm Controller v0.20.0

> **Current development: v0.20.0 diagnostic logging.** Controller-wide structured diagnostic logs are stored independently under `logs/` by default, with `LOG_DIR` available separately from `DATA_DIR` for container/PV deployments. The overflow menu now includes **Diagnostics** for recent-log viewing/filtering, temporary 30-minute verbose DEBUG logging, and downloading a sanitized ZIP support bundle. Credential-like fields are redacted, print files and licence contents are excluded, and packaged Windows installs grant normal-user write access to both `data/` and `logs/`.
