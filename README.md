...
> **Current development: v0.23.0 backup and recovery.** Portable `.pfcbackup` logical snapshots, manifest/checksum verification, the manual backup API, one-time streamed download, and the Backup & Recovery UI are now implemented on `feature/backup-recovery-v0230`. The next stage is restore inspection/staging with atomic restart activation and recovery-held unfinished queue work. Scheduled local/NAS backups follow after restore. Cloud destinations and built-in backup encryption are deferred beyond v0.23.0. See `docs/BACKUP_RECOVERY.md`.
