# Print Farm Controller v0.23.0

> **Current development: v0.23.0 backup and recovery.** Portable `.pfcbackup` backup creation, manual download, read-only inspection, staged restart-based restore/rollback, and scheduled local/network/NAS backups are implemented on `feature/backup-recovery-v0230`. Scheduled backups support daily or weekly controller-local times, missed-run catch-up shortly after controller startup, explicit destination writability testing, local folders/mapped drives/UNC paths, shared locking against manual backups, status/next-run reporting, and count-based retention. Retention runs only after a new scheduled backup is fully verified and only prunes scheduler-owned backups from the same installation; manual and foreign-installation backups are never auto-deleted. Restore continues to recovery-hold unfinished work and retain the pre-restore snapshot for 24 hours after successful activation. The final end-to-end disaster-recovery integration test is now implemented, covering backup from one installation, restore into a fresh installation, service reinitialization, Print Library hashes/metadata, queue/history safety, restored schedule disablement, emulator settings and normal licence revalidation. Remaining v0.23.0 work is full Windows automated/UI/live validation before release readiness. Cloud destinations and built-in backup encryption remain deferred. See `docs/BACKUP_RECOVERY.md`.
