...
> **Current development: v0.23.0 backup and recovery.** The design is defined in `docs/BACKUP_RECOVERY.md`. The feature will create portable `.pfcbackup` logical snapshots of controller-owned persistent state, support manual and scheduled local/NAS backups, validate checksums before restore, activate restores atomically on restart with rollback, and recovery-hold unfinished queue work so a restored controller can never automatically resume a print. Cloud destinations and built-in backup encryption are deferred beyond v0.23.0.
