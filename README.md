...
> **Current development: v0.23.0 backup and recovery.** Portable `.pfcbackup` backup creation, manual download, read-only inspection, staged restore, restart-time activation and automatic rollback are implemented on `feature/backup-recovery-v0230`. Restore staging is blocked while physical work is active, temporarily pauses queue dispatch, freezes controller mutations after staging, recovery-holds all unfinished jobs, pauses restored production batches, preserves/forces bed-clearance interlocks where a print may have started, and rechecks live compatibility before release. A failed or interrupted restored startup rolls back automatically; after a successful restored startup, the pre-restore snapshot is retained for a 24-hour recovery window. Remaining v0.23.0 work is scheduled local/network/NAS backup, retention/status, and full disaster-recovery integration coverage. Cloud destinations and built-in backup encryption remain deferred. See `docs/BACKUP_RECOVERY.md`.
