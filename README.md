...
> **Current development: v0.24.0 maintenance tracking.** `feature/maintenance-tracking-v0240` adds controller-owned recurring maintenance schedules for each printer, with calendar-day, controller-observed print-hour, and observed print-cycle intervals. The Maintenance view shows current/due-soon/due tasks, observed usage, editable task instructions and recent completion history. Completing a task records an immutable history entry with optional notes and a usage snapshot before resetting that task's interval baseline. Maintenance data is persisted in `data/maintenance.json` and is included in `.pfcbackup` backup/restore. Observed usage begins when this controller tracks the printer and is intentionally labelled controller-observed rather than manufacturer lifetime usage.
...
- **Maintenance tracking** is available from the controller overflow menu. Each printer can have recurring service tasks based on calendar days, controller-observed print hours, or observed print cycles. Tasks show Current / Due soon / Due state, can be enabled/disabled or edited, and retain completion history with optional notes and usage snapshots. The controller stores this separately from printer firmware so the workflow is manufacturer-agnostic.
