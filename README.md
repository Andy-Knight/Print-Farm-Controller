# Print Farm Controller v0.25.1

> **Current development: v0.25.1 dashboard group filtering.** `feature/dashboard-group-filter-v0251` adds a **Printer group** selector to the main dashboard. Choosing a custom group restricts the visible printer cards to that group's current members, while the existing Printers / Online / Printing / Needs attention / Maintenance filters continue to work within the selected group. Summary counts and the maintenance-alert filter reflect the selected group, group membership changes update the dashboard selector/filter immediately, and **Show all printers** clears both the status and group filters.
