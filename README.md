# Print Farm Controller v0.19.0

> **Current development: v0.19.0 printer activity status messaging.** Printer detail windows now use the prominent activity banner introduced for Snapmaker U1 bed levelling as a general controller status surface. Controller-tracked bed levelling and calibration can be shown consistently across supported printer types, while chamber preheat is surfaced at the top of the printer window without duplicating its existing dashboard preheat strip.
